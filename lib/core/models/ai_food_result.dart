...
class AIFoodResult {
  ...
  factory AIFoodResult.fromJson(Map<String, dynamic> json) {
    ...
    if (json["foods"] is List && (json["foods"] as List).isNotEmpty) {
      food = (json["foods"] as List).first as Map<String, dynamic>;
    } else {
      food = json;
    }
    ...
    ScanUsage? usage;
    if (json["scanUsage"] is Map<String, dynamic>) {
      usage = ScanUsage.fromJson(json["scanUsage"] as Map<String, dynamic>);
    }
    ...
    return AIFoodResult(
      ...
      quantity: numToDouble(food["estimatedWeight"] ?? food["quantity"] ?? 100),
      confidence: numToDouble(food["confidenceScore"] ?? food["confidence"] ?? 0.9),
      servingSize: food["servingSize"]?.toString() ?? "${food["estimatedWeight"] ?? 100} g",
      healthyScore: (json["healthyScore"] as num?)?.toInt() ?? 80,
      scanUsage: usage,
    );
  }
}
