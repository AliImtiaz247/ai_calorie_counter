...
class _ScanFoodScreenState extends State<ScanFoodScreen> {
  ...
  Future analyzeFood() async {
    ...
    try {
      ...
      // Refresh scan usage when returning from result screen
      _fetchScanUsage();
    } on ScanLimitException catch (limitErr) {
      if (!mounted) return;
      setState(() {
        _scanUsage = limitErr.scanUsage;
      });
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            "${LanguageService.tr('Analysis failed')}: ${e.toString().replaceAll('Exception:', '')}",
          ),
          backgroundColor: Colors.redAccent,
        ),
      );
                            ...
                            onPressed: () async {
                              ...
                              if (!hasNet) {
                                messenger.showSnackBar(
                                  SnackBar(
                                    content: Text(
                                      LanguageService.tr("Still offline. Please check connection."),
                                    ),
                                    backgroundColor: Colors.redAccent,
                                  ),
                                );
  ...
  Widget _buildScanUsageBadge(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(20),
        border: Border.all(
          color: isLimitReached ? const Color(0xFFEF4444).withValues(alpha: 0.5) : Colors.grey.withValues(alpha: 0.2),
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      fontSize: 15,
                      fontWeight: FontWeight.bold,
                      color: isLimitReached ? const Color(0xFFEF4444) : Theme.of(context).colorScheme.onSurface,
                    ),
                  ),
                ],
              ),
                  ...
                  child: image == null
                      ? Container(
                          ...
                          child: Center(
                            child: Text(
                              LanguageService.tr(
                                  'No image selected yet. Tap the camera button to capture your meal.'),
                              textAlign: TextAlign.center,
                              style: const TextStyle(
                                fontSize: 14,
  ...
  Widget _buildActionPanel(BuildContext context) {
    final isLimitReached = (_scanUsage?.remaining ?? 5) <= 0;
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: loading
                ...
                : Text(
                    isLimitReached
                        ? LanguageService.tr('Daily Limit Reached (5/5)')
